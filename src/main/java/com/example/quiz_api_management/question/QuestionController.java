...
public class QuestionController {
    ...
    public ResponseEntity<ResponseReturn> getQuestionsByQuiz(@PathVariable("quizid") int quizId,
        ...
        if (order == null && filter == null) {
            ...
            List<QuestionDTO> sortedQuestions = questionService.sortQuestions(quiz, order);
            return new ResponseEntity<>(
                    new ResponseReturn(LocalDateTime.now(),
                            "List of sorted questions is returned.",
                            HttpStatus.OK.value(),
                            true,
                            sortedQuestions), HttpStatus.OK);
        }

        else if (order == null) {
            List<QuestionDTO> filteredQuestions = questionService.filterQuestionsByType(quiz, filter);
            return new ResponseEntity<>(
                    new ResponseReturn(LocalDateTime.now(),
                            "List of filtered questions is returned.",
                            HttpStatus.OK.value(),
                            true,
                            filteredQuestions), HttpStatus.OK);
        }

        else {
            List<QuestionDTO> filteredAndSortedQuestions = questionService.filterAndSortQuestions(quiz, filter, order);
            return new ResponseEntity<>(new ResponseReturn(
                    LocalDateTime.now(),
                    "Return list of questions which is filtered and sorted.",
                    HttpStatus.OK.value(),
                    true,
                    filteredAndSortedQuestions), HttpStatus.OK);
        }
    }
    ...
    public ResponseEntity<ResponseReturn> deleteQuestion(@PathVariable("questionid") int questionId){
        Optional.of(questionService.getQuestion(questionId).orElseThrow(()
